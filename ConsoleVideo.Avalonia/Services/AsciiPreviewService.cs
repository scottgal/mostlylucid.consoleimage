...
public class AsciiPreviewService
{
    ...
    public List<AnsiSegment> ParseAnsiToSegments(string ansiText)
    {
        ...
        if (currentText.Length > 0) segments.Add(new AnsiSegment(currentText.ToString(), currentColor));
    ...
    {
        if (sequence == "0" || string.IsNullOrEmpty(sequence)) return new AnsiColor(192, 192, 192); // Reset to default
