...
public class VideoPreviewService : IDisposable
{
    ...
    /// </summary>
    public VideoInfo? CurrentVideo { get; private set; }
    ...
    public async Task<VideoInfo?> LoadVideoAsync(string path, CancellationToken ct = default)
    {
        ...
        CurrentVideo = await _ffmpeg.GetVideoInfoAsync(path, ct);
        ...
        CancellationToken ct = default)
    {
        ...
        if (_currentVideoPath == null || CurrentVideo == null)
            return results;
        ...
        for (var i = 1; i <= thumbnailCount; i++)
        {
            ...
            if (bitmap != null) results.Add((timestamp, bitmap));
        }
